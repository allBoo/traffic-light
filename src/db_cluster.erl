...
start_database() ->
  DataPath = filename:absname(config:get("path.data", "/var/lib/" ++ env:app(string), string)),
        ...
        false ->
          case file:make_dir(DataPath) of
            ...
            {error, Reason} ->
              ...
              ?THROW_ERROR({error, "Data path is not accessible"})
          end
      end;
    {error, Reason} ->
...
try_to_add_node(Node) ->
  ...
  case mnesia:change_config(extra_db_nodes, [Node]) of
    ...
    Error ->
      ...
      ?THROW_ERROR({error,  "Error while connecting to a new database"})
  end.
