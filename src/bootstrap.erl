...
init_logs() ->
  ...
  %% add file handler
  add_file_handler(log_lager_event, config:get('log.file', env:app(string) ++ ".log", string), LogLevel),
...
start_crash_log(File) ->
  LogFile = filename:absname(config:get('path.logs', "/var/log/" ++ env:app(string), string) ++ "/" ++ File),
